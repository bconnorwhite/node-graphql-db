...
import { typeNameToWhereInput } from 'essence-tools';

import where, { WhereInputArgs } from './where';
import { whereUniqueByTable, whereUniqueByNodeList } from './whereUnique';
...
export function fieldValueIsNodeArray(fieldValue: FieldValue): fieldValue is Node[] {
  return Array.isArray(fieldValue as Node[]);
}

export function fieldValueIsNode(fieldValue: FieldValue): fieldValue is Node {
  return (fieldValue as Node).uuid !== undefined;
}

export function fieldValueIsTypeFieldValue(fieldValue: FieldValue): fieldValue is TypeFieldValue {
  return (fieldValue as TypeFieldValue) == null || fieldValueIsNode(fieldValue as TypeFieldValue) || fieldValueIsNodeArray(fieldValue as TypeFieldValue);
}
...
export default class Database {
  ...
  whereUnique(typeName: string, where: object) {
    ...
    return whereUniqueByTable(table, where);
  }
  ...
  whereUniqueByNodeList(nodes: (Node | Node[]), where: object) {
    return whereUniqueByNodeList(nodes, where);
  }
  ...
  create(typeName: string, data: (object | object[])) {
  ...
  update(typeName: string, nodes: (Node | Node[]), data: object) {
    let result = update(this, typeName, nodes, data);
    if(result) {
      this.commit(result.commit);
      return result.node;
    } else {
  ...
  destroy(typeName: string, nodes: (Node | Node[])) {
    let result = destroy(this, typeName, nodes);
    if(result) {
      this.commit(result.commit);
      return result.node;
    } else {
